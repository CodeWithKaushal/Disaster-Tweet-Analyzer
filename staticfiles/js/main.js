        ...
        if (data.is_disaster) {
            resultHTML = `
                <div class="alert alert-danger" role="alert">
                    ...
                    <hr>
                    <p class="mb-0">Confidence: ${(data.confidence * 100).toFixed(2)}%</p>
                </div>
            `;
            ...
            if (data.locations && data.locations.length > 0) {
                resultHTML += `
                    <div class="card mt-3">
                        ...
                        <div class="card-body">
                            <ul class="list-group">
                                ${data.locations.map(loc => `<li class="list-group-item">${loc}</li>`).join('')}
                            </ul>
                        </div>
                    </div>
                `;
        ...
        if (data.sentiment) {
            resultHTML += `
                <div class="card mt-3">
                    ...
                    <div class="card-body">
                        <p class="card-text">Sentiment: ${data.sentiment.label} (${(data.sentiment.score * 100).toFixed(2)}%)</p>
                    </div>
                </div>
            `;
        }
