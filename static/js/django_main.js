...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateResults(data) {
    ...
    const predictionProgressBar = document.getElementById(
    ...
    if (predictionConfidence)
      predictionConfidence.textContent = `${probability}% confidence`;
    if (predictionProgressBar) {
      ...
      if (prediction === "Disaster") {
        predictionProgressBar.className =
    ...
    if (positiveSentiment)
      positiveSentiment.textContent = `${(data.sentiment.pos * 100).toFixed(
        2
      )}%`;
    if (neutralSentiment)
      neutralSentiment.textContent = `${(data.sentiment.neu * 100).toFixed(
        2
      )}%`;
    if (negativeSentiment)
      negativeSentiment.textContent = `${(data.sentiment.neg * 100).toFixed(
        2
      )}%`;
    if (compoundSentiment)
      compoundSentiment.textContent = data.sentiment.compound.toFixed(2);
